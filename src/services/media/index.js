...
mediaRouter.get("/", async (req, res, next) => {
  try {
    const authors = await readFile("media.json");
    res.send(authors);
  } catch (error) {
    next(error);
  }
...
//2, GET SINGLE author
mediaRouter.get("/:authorId", async (req, res, next) => {
  try {
...
//  4. PUT Single author
mediaRouter.put("/:authorId", async (req, res, next) => {
  try {
    const authors = await readFile("authors.json");
...
//5. DELETE  author
mediaRouter.delete("/:authorId", async (req, res, next) => {
  try {
    const authors = await readFile("authors.json");
    const remainingAuthors = authors.filter(
      (author) => author._id !== req.params.authorId
...
//POST auhthor avatar
mediaRouter.post("/:authorId/uploadAvatar", async (req, res, next) => {
  try {
...
//update avatar
mediaRouter.put("/:authorId/avatar", async (req, res, next) => {
  try {
